...
const NavBar = ({ sections }) => {
  return (
    <div className="flex flex-row w-full bg-black gap-3 shadow-[0_-12px_100px_28px_black]" id="Navbar">
      <ul className="flex flex-row justify-start items-center w-full text-white my-4 2xl:my-5 font-sans text-mg sm:text-lg xl:text-2xl">
        {Object.entries(sections).map(([ref, name]) => (
      ...
      <div className="flex">
        <LangSwitcher className="mr-5"/>
        <SocialMedia
          className="self-center mr-6 items-center max-[425px]:hidden"
          mediaUrls={MediaUrls}
        />
      </div>
    </div>
